...
(function () {
  ...
  BBPlayer.prototype.loadSources = function () {
    ...
    [].forEach.call(
      ...
      function (elem) {
        ...
        var trackName = elem.getAttribute('audio-title');//fileName.split('.').shift();
  ...
  BBPlayer.prototype.updateDisplay = function () {
    ...
    var srcElem = audioElem.getElementsByTagName("source").item(0);
    var duration  = toTimeString(Math.ceil(audioElem.duration));
    var elapsed   = toTimeString(Math.ceil(audioElem.currentTime));
    var title     = srcElem.getAttribute('audio-title');//parseTitle(audioElem.currentSrc);
